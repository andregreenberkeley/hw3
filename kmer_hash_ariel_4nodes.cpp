...
int main(int argc, char** argv) {
    ...
    // Load factor of 0.25 to reduce collisions
    size_t hash_table_size = n_kmers * 4;
    ...
    for (auto& kmer : kmers) {
        ...
        if (!success) {
            if (run_type == "verbose") {
                BUtil::print("Rank %d: Failed to insert k-mer.\n", upcxx::rank_me());
            }
    ...
    hashmap.flush_insertions(); // Ensure all insertions are complete
    ...
    for (const auto& start_kmer : start_nodes) {
        ...
        int steps = 0;
        const int max_steps = 10000; // Prevent infinite loops
        ...
        if (steps >= max_steps && run_type == "verbose") {
